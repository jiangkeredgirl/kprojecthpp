...
inline static void mySystem(const string& cmd, const string& par = string(), int nShow = SW_HIDE, bool is_block = false)
{
    ...
    ShExecInfo.lpVerb = "runas";          // 关键参数：请求管理员权限
...
inline static QImage rgbMatToQImage(cv::Mat cvMat)
{
    switch (cvMat.type()) {
    case CV_8UC4:{
...
inline static QImage bgrMatToQImage(const cv::Mat& mat) {
    // 8-bit, 4 channel
    if(mat.type() == CV_8UC4) {
...
inline static bool QRCodeCheck(const QString &text)
{
    bool bState;
...
inline static QSize CurrentScreenSize(QWidget *window) {

    QSize screen_size;
    QScreen *screen = window->screen();
    if (screen) {
        QSize size = screen->availableGeometry().size();  // 可用区域（不含任务栏）
        QSize fullSize = screen->geometry().size();        // 全屏尺寸（含任务栏）
        screen_size = size;
        qDebug() << "当前屏幕可用尺寸：" << size.width() << "x" << size.height();
        qDebug() << "当前屏幕总尺寸：" << fullSize.width() << "x" << fullSize.height();
    }
    return screen_size;
}
