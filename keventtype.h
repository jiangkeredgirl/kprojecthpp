...
/// 针尖位置检测设备获取图像操作事件
struct NeedlePosDetectionDeviceFrameOperaEventParam : public DeviceOperaEventParam
{
    NeedlePosDetectionFrame frame;
    NeedlePosDetectionDeviceFrameOperaEventParam()
    {
        //event_id = NIRDEVICE_OPERAID_FRAME;
        opera_id = NEEDLEPOSDETECTIONDEVICE_OPERAID_FRAME;
        event_name = typeid(*this).name();
    }
    virtual ~NeedlePosDetectionDeviceFrameOperaEventParam()
    {
    }
    virtual string EventStr() const  override
    {
        return DeviceOperaEventParam::EventStr()
               + "\n, frame  width:" + to_string(frame.mat_frame.cols)
               + "\n, frame height:" + to_string(frame.mat_frame.rows)
            ;
    }
};

struct NeedlePosDetectionDeviceCaptureFrameOperaEventParam : public DeviceOperaEventParam
{
    NeedlePosDetectionCaptureFrame frame;
    NeedlePosDetectionDeviceCaptureFrameOperaEventParam()
    {
        //event_id = NIRDEVICE_OPERAID_FRAME;
        opera_id = NEEDLEPOSDETECTIONDEVICE_OPERAID_CAPTURE;
        event_name = typeid(*this).name();
    }
    virtual ~NeedlePosDetectionDeviceCaptureFrameOperaEventParam()
    {
    }
    virtual string EventStr() const  override
    {
        return DeviceOperaEventParam::EventStr()
               + "\n, frame  width:" + to_string(!frame.mat_horizontal.empty() ? frame.mat_horizontal.cols : frame.mat_tilt.cols)
               + "\n, frame height:" + to_string(!frame.mat_horizontal.empty() ? frame.mat_horizontal.rows : frame.mat_tilt.rows)
            ;
    }
};

/// 激光测距设备测距操作事件
struct LaserDistanceDeviceRangingOperaEventParam : public DeviceOperaEventParam
{
    double distance = 0;
    LaserDistanceDeviceRangingOperaEventParam()
    {
        //event_id = NIRDEVICE_OPERAID_FRAME;
        opera_id = LASERDISTANCEDEVICE_OPERAID_DISTANCE;
        event_name = typeid(*this).name();
    }
    virtual ~LaserDistanceDeviceRangingOperaEventParam()
    {
    }
    virtual string EventStr() const  override
    {
...
/// 大恒设备获取图像操作事件
struct DahengDeviceFrameOperaEventParam : public DeviceOperaEventParam
{
    DahengFrame frame;
    DahengDeviceFrameOperaEventParam()
    {
        opera_id   = DAHENGDEVICE_OPERAID_FRAME;
        event_name = typeid(*this).name();
    }
    virtual ~DahengDeviceFrameOperaEventParam()
    {
...
/// 大恒设备获取图像操作事件
struct DahengDeviceCaptureFrameOperaEventParam : public DeviceOperaEventParam
{
    DahengCaptureFrame capture_frame;
    DahengDeviceCaptureFrameOperaEventParam()
    {
        opera_id   = DAHENGDEVICE_OPERAID_CAPTURE;
        event_name = typeid(*this).name();
    }
    virtual ~DahengDeviceCaptureFrameOperaEventParam()
    {
